...
1. 新建项目的文件夹, 如demo, demo下包含`test.rb`文件
2. 在VSCode中点击文件->打开文件夹->选择demo
3. 在demo下新建`.vscode`目录
4. 在`demo/.vscode`下新建`launch.json`文件
5. 在`launch.json`文件中输入以下内容：
...
在vscode中安装cucumber插件后，在`demo/.vscode`下新建`setting.json`文件。
